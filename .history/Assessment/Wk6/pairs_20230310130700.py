...
def print_grid(d_grid, i_clear=0, i_reveal = 0 ):
    ...
    for i in range(4):
        print ( " " * 5 + str(i) + " |", end = " " )
        for j in range(4):
            if  d_grid[i][j]["check"]==1 or i_reveal==1:
                print (d_grid[i][j]["val"], end=" ")
            elif d_grid[i][j]["matched"]==1:
                print ("X", end=" ")
            else:
                print ( "#", end = " " )
        print ("|")
...
def get_location(d_grid):
    while True:
        try: 
            ...
            if d_grid[y1][x1]["matched"]==1:
                print ( f"Location {loc1} is already matched " )
                raise Exception
            if d_grid[y1][x1]["check"]==1:
...
while i_matches < 8:
    ...
    d_grid[y2][x2]["check"]=1
    print_grid ( d_grid, 1 )

    if d_grid[y2][x2]["val"] == d_grid[y1][x1]["val"]:
        d_grid[y1][x1]["matched"]=1
        d_grid[y2][x2]["matched"]=1
        i_matches += 1
    ...
    d_grid[y1][x1]["check"]=0
    d_grid[y2][x2]["check"]=0

    x = input ("Press ENTER to continue")

print_grid( d_grid, 1, 1)
if i_matches==8:
    print ( f"Congratulations. You cleared the board in {i_guesses} rounds! ")
else: 
