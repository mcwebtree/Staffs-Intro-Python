...
def create_ships():
    ...
    ships_to_place = num_ships.count()
    ships_placed = 0
...
def find_space(length):
    ...
    i_iters=0
    i_max_iters = 1000
    start_x = r.randrange(0, board_width)
    start_y = r.randrange(1, board_height)
    x, y = start_x, start_y
    ...
    while True:
        ...
        if targets[start_x][start_y]==" ":
            ...
            start_x = r.randrange(0, board_width)
            start_y = r.randrange(0, board_height)
            dir = r.choice( ['N', 'E', 'S', 'W'] )
            found_spaces = 0
            done_swap = 0 
            i_iters += 1
...
if s_game_type == "S":
    ...
    user_board = input("Please enter the board dimensions (e.g. 5x5, 4x9): ")
    board_width, board_height = user_board.split("x")                       
