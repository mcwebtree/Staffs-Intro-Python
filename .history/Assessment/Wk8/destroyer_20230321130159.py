...
def print_board(f_live = 1):
    ...
    s_output = s_output.replace( "[@]" , REVON + '[@]'+ REVOFF )
    print ( s_output )
...
def place_ship(a_ship):
    ...
    for plot in a_ship["Locn"]:
        targets[plot[0]][plot[1]]=ship_ids[this_ship]
        parts_to_sink +=1
    ...
    for ship in auto_ships:
        if ship["Length"] > i_max_size:
            ...
            if ship_locn['Success']==1:
                ships.append ( {"Length": ship['Length'], "Locn": ship_locn['Locn']} )
...
if s_game_type == "S":
    ...
    ships.append({"Length": 2, "Locn": [[2,2],[2,3]], "Code": "A"})
else:
