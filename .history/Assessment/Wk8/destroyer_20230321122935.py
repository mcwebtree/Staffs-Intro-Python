...
def find_space(length):
    ...
    while True:
        ...
        if f_debug == 1:
            print ( f'Testing {start_c},{start_r}' )

        if targets[start_c][start_r]=="M":
            ret_val['Locn'].append([start_c, start_r])
            ...
            while found_spaces < length:
                ...
                if xc < 0 or xc >= board_width or yr < 0 or yr >= board_height or targets[yr][xc] == "H":
                    done_swap += 1
...
if s_game_type == "S":
    ...
    user_board = input("Please enter the board dimensions (e.g. 5x5, 4x9) [MIN 4x4, MAX 9x9]: ")
